...
#define poly1305_shift_flags (poly1305_final_shift8  |  \
                              poly1305_final_shift16 |  \
                              poly1305_final_shift24 |  \
                              poly1305_final_shift32)

#define poly1305_mult_flags (poly1305_final_r4_r4_r4_r3 |          \
                             poly1305_final_r4_r4_r3_r2 |          \
                             poly1305_final_r4_r3_r2_r  |          \
                             poly1305_final_r3_r2_r_1   |          \
                             poly1305_final_r2_r_1_1    |          \
                             poly1305_final_r_1_1_1)

struct poly1305_state_internal {
	union {
		uint32_t h[5];
		uint32_t hh[20];
	};                       /*  80 bytes  */
...
typedef struct poly1305_state_internal poly1305_state_internal_t;
                      ...
                      size_t bytes)
{
	...
	if (bytes & 32) {
                _mm256_store_si256((ymmi *) dst, _mm256_loadu_si256((ymmi *) (dst + offset)));
                dst += 32;
        }
	if (bytes & 16) {
                _mm_store_si128((xmmi *) dst, _mm_loadu_si128((xmmi *) (dst + offset)));
                dst += 16;
        }
	if (bytes &  8) {
...
void
FN(poly1305_init_ext)(poly1305_state_internal_t *st,
                      ...
                      size_t bytes)
{
	...
	/* clamp key */
	t0 = *(const uint32_t *) (key + 0);
	t1 = *(const uint32_t *) (key + 4);
	t2 = *(const uint32_t *) (key + 8);
	t3 = *(const uint32_t *) (key + 12);
	r0 = t0 & 0x3ffffff;
        t0 >>= 26;
        t0 |= t1 << 6;
	...
	/* save pad */
	st->pad[0] = *(const uint32_t *) (key + 16);
	st->pad[1] = *(const uint32_t *) (key + 20);
	st->pad[2] = *(const uint32_t *) (key + 24);
	st->pad[3] = *(const uint32_t *) (key + 28);

	if (bytes > 16) {
		const xmmi FIVE = _mm_shuffle_epi32(_mm_cvtsi32_si128(5),
                                                    _MM_SHUFFLE(1, 0, 1, 0));
		...
		v31 = _mm_mul_epu32(_mm_add_epi32(R23, R23), R20);
                T0 = _mm_add_epi64(T0, v03);
		v42 = _mm_mul_epu32(_mm_add_epi32(R20, R20), R24);
                T1 = _mm_add_epi64(T1, v12);
		v01 = _mm_mul_epu32(R20, R20);
                T2 = _mm_add_epi64(T2, v22);
		v11 = _mm_mul_epu32(_mm_add_epi32(R21, R21), R20);
                T3 = _mm_add_epi64(T3, v31);
		v21 = _mm_mul_epu32(R21, R21);
                T4 = _mm_add_epi64(T4, v42);
		v32 = _mm_mul_epu32(_mm_add_epi32(R21, R21), R22);
                T0 = _mm_add_epi64(T0, v01);
		v41 = _mm_mul_epu32(R22, R22);
		...
		if (bytes > 32) {
			...
			T2  = R24; T2  = _mm_mul_epu32(T2 , T14);
                        T0 = _mm_add_epi64(T0, v01);
			T15 = S44;
			v02 = R22; v02 = _mm_mul_epu32(v02, T14);
			T3  = R24; T3  = _mm_mul_epu32(T3 , T15);
                        T1 = _mm_add_epi64(T1, v11);
			v03 = R21; v03 = _mm_mul_epu32(v03, T15);
			v12 = R22; v12 = _mm_mul_epu32(v12, T15);
                        T0 = _mm_add_epi64(T0, v02);
			T14 = R40;
			v21 = R23; v21 = _mm_mul_epu32(v21, T15);
			v31 = R23; v31 = _mm_mul_epu32(v31, T14);
                        T0 = _mm_add_epi64(T0, v03);
			T4  = R24; T4  = _mm_mul_epu32(T4 , T14);
                        T1 = _mm_add_epi64(T1, v12);
			v04 = R20; v04 = _mm_mul_epu32(v04, T14);
                        T2 = _mm_add_epi64(T2, v21);
			v13 = R21; v13 = _mm_mul_epu32(v13, T14);
                        T3 = _mm_add_epi64(T3, v31);
			T15 = R41;
			v22 = R22; v22 = _mm_mul_epu32(v22, T14);
			v32 = R22; v32 = _mm_mul_epu32(v32, T15);
                        T0 = _mm_add_epi64(T0, v04);
			v41 = R23; v41 = _mm_mul_epu32(v41, T15);
                        T1 = _mm_add_epi64(T1, v13);
			v14 = R20; v14 = _mm_mul_epu32(v14, T15);
                        T2 = _mm_add_epi64(T2, v22);
			T14 = R42;
			v23 = R21; v23 = _mm_mul_epu32(v23, T15);
                        T3 = _mm_add_epi64(T3, v32);
			v33 = R21; v33 = _mm_mul_epu32(v33, T14);
                        T4 = _mm_add_epi64(T4, v41);
			v42 = R22; v42 = _mm_mul_epu32(v42, T14);
                        T1 = _mm_add_epi64(T1, v14);
			T15 = R43;
			v24 = R20; v24 = _mm_mul_epu32(v24, T14);
                        T2 = _mm_add_epi64(T2, v23);
			v34 = R20; v34 = _mm_mul_epu32(v34, T15);
                        T3 = _mm_add_epi64(T3, v33);
			v43 = R21; v43 = _mm_mul_epu32(v43, T15);
                        T4 = _mm_add_epi64(T4, v42);
			v44 = R20; v44 = _mm_mul_epu32(v44, R44);
...
void
FN(poly1305_blocks)(poly1305_state_internal_t *st,
                    const uint8_t *m,
                    size_t bytes)
{
	...
	if (st->flags & poly1305_shift_flags) {
		...
		if (st->flags & poly1305_final_shift8)
                        T0 = _mm256_permute4x64_epi64(T0, _MM_SHUFFLE(3, 0, 0, 0));
		if (st->flags & poly1305_final_shift16)
                        T0 = _mm256_permute4x64_epi64(T0, _MM_SHUFFLE(3, 3, 0, 0));
		if (st->flags & poly1305_final_shift24)
                        T0 = _mm256_permute4x64_epi64(T0, _MM_SHUFFLE(3, 3, 3, 0));
		if (st->flags & poly1305_final_shift32)
	...
	if (!(st->flags & poly1305_started)) {
		/* H = [Mx,My] */
		T7 = _mm256_loadu_si256((const ymmi *) (m + 0));
		T8 = _mm256_loadu_si256((const ymmi *) (m + 32));
	...
	if (bytes >= 64) {
		...
		do {
			...
			T2  = H4; T2  = _mm256_mul_epu32(T2 , T14);
                        T0 = _mm256_add_epi64(T0, v01);
			T15 = S44;
			v02 = H2; v02 = _mm256_mul_epu32(v02, T14);
			T3  = H4; T3  = _mm256_mul_epu32(T3 , T15);
                        T1 = _mm256_add_epi64(T1, v11);
			v03 = H1; v03 = _mm256_mul_epu32(v03, T15);
			v12 = H2; v12 = _mm256_mul_epu32(v12, T15);
                        T0 = _mm256_add_epi64(T0, v02);
			T14 = R40;
			v21 = H3; v21 = _mm256_mul_epu32(v21, T15);
			v31 = H3; v31 = _mm256_mul_epu32(v31, T14);
                        T0 = _mm256_add_epi64(T0, v03);
			T4  = H4; T4  = _mm256_mul_epu32(T4 , T14);
                        T1 = _mm256_add_epi64(T1, v12);
			v04 = H0; v04 = _mm256_mul_epu32(v04, T14);
                        T2 = _mm256_add_epi64(T2, v21);
			v13 = H1; v13 = _mm256_mul_epu32(v13, T14);
                        T3 = _mm256_add_epi64(T3, v31);
			T15 = R41;
			v22 = H2; v22 = _mm256_mul_epu32(v22, T14);
			v32 = H2; v32 = _mm256_mul_epu32(v32, T15);
                        T0 = _mm256_add_epi64(T0, v04);
			v41 = H3; v41 = _mm256_mul_epu32(v41, T15);
                        T1 = _mm256_add_epi64(T1, v13);
			v14 = H0; v14 = _mm256_mul_epu32(v14, T15);
                        T2 = _mm256_add_epi64(T2, v22);
			T14 = R42;
			v23 = H1; v23 = _mm256_mul_epu32(v23, T15);
                        T3 = _mm256_add_epi64(T3, v32);
			v33 = H1; v33 = _mm256_mul_epu32(v33, T14);
                        T4 = _mm256_add_epi64(T4, v41);
			v42 = H2; v42 = _mm256_mul_epu32(v42, T14);
                        T1 = _mm256_add_epi64(T1, v14);
			T15 = R43;
			v24 = H0; v24 = _mm256_mul_epu32(v24, T14);
                        T2 = _mm256_add_epi64(T2, v23);
			v34 = H0; v34 = _mm256_mul_epu32(v34, T15);
                        T3 = _mm256_add_epi64(T3, v33);
			v43 = H1; v43 = _mm256_mul_epu32(v43, T15);
                        T4 = _mm256_add_epi64(T4, v42);
			v44 = H0; v44 = _mm256_mul_epu32(v44, R44);
                        T2 = _mm256_add_epi64(T2, v24);
                        T3 = _mm256_add_epi64(T3, v34);
                        T4 = _mm256_add_epi64(T4, v43);
                        T4 = _mm256_add_epi64(T4, v44);

			/* H += [Mx,My] */
			T5 = _mm256_loadu_si256((const ymmi *)(m + 0));
			T6 = _mm256_loadu_si256((const ymmi *)(m + 32));
			...
			/* reduce */
			C1 = _mm256_srli_epi64(T0, 26);
                        C2 = _mm256_srli_epi64(T3, 26);
                        T0 = _mm256_and_si256(T0, MMASK);
                        T3 = _mm256_and_si256(T3, MMASK);
                        T1 = _mm256_add_epi64(T1, C1);
                        T4 = _mm256_add_epi64(T4, C2); 
			C1 = _mm256_srli_epi64(T1, 26);
                        C2 = _mm256_srli_epi64(T4, 26);
                        T1 = _mm256_and_si256(T1, MMASK);
                        T4 = _mm256_and_si256(T4, MMASK);
                        T2 = _mm256_add_epi64(T2, C1);
                        T0 = _mm256_add_epi64(T0, _mm256_mul_epu32(C2, FIVE)); 
			C1 = _mm256_srli_epi64(T2, 26);
                        C2 = _mm256_srli_epi64(T0, 26);
                        T2 = _mm256_and_si256(T2, MMASK);
                        T0 = _mm256_and_si256(T0, MMASK);
                        T3 = _mm256_add_epi64(T3, C1);
                        T1 = _mm256_add_epi64(T1, C2);
			C1 = _mm256_srli_epi64(T3, 26);
                        T3 = _mm256_and_si256(T3, MMASK);
                        T4 = _mm256_add_epi64(T4, C1);
	...
	if (!(st->flags & poly1305_finalize)) {
		...
		t0 = _mm_cvtsi128_si32(_mm256_castsi256_si128(T0));
                c = (t0 >> 26);
                t0 &= 0x3ffffff;
                
		t1 = _mm_cvtsi128_si32(_mm256_castsi256_si128(T1)) + c;
                c = (t1 >> 26);
                t1 &= 0x3ffffff;
                
		t2 = _mm_cvtsi128_si32(_mm256_castsi256_si128(T2)) + c;
                c = (t2 >> 26);
                t2 &= 0x3ffffff;
                
		t3 = _mm_cvtsi128_si32(_mm256_castsi256_si128(T3)) + c;
                c = (t3 >> 26);
                t3 &= 0x3ffffff;
                
		t4 = _mm_cvtsi128_si32(_mm256_castsi256_si128(T4)) + c;
                c = (t4 >> 26);
                t4 &= 0x3ffffff;
...
void
FN(poly1305_finish_ext)(poly1305_state_internal_t *st,
                        const uint8_t *m,
                        size_t leftover,
                        unsigned char mac[16])
{
	...
	if (leftover) {
		...
		if (leftover >= 48)
                        st->flags |= poly1305_final_shift8;
		else if (leftover >= 32)
                        st->flags |= poly1305_final_shift16;
		else if (leftover >= 16)
                        st->flags |= poly1305_final_shift24;
		else
                        st->flags |= poly1305_final_shift32;
...
void
FN(poly1305_auth)(unsigned char out[16],
                  const unsigned char *m,
                  size_t inlen,
                  const unsigned char key[32])
{
	poly1305_state_internal_t st __attribute__((aligned(64)));
	size_t blocks;
